  ...
  var COUNT =
      animation > 1 ||
          whitelistingClicked && whitelistingClicked-- && !(
            CONTENT &&
                $('.whitelisting').filter('.text').text() == 'Unblock site'
          ) ? 21 : animation;
...
function renderWhitelisting(siteWhitelist) {
  ...
  if (
    ...
    WHITELISTING_ICON.alt = 'Block';
    WHITELISTING_TEXT.text('Block site');
  } else {
    WHITELISTING_ICON.alt = 'Unblock';
    WHITELISTING_TEXT.text('Unblock site');
  }
...
(SAFARI ? safari.application : window).addEventListener(
  SAFARI ? 'popover' : 'load', function() {
    ...
    if (!displayMode || displayMode == LEGACY) {
      ...
      */
      var GPC = $('.gpc ' + INPUT)[0];
      GPC.checked = DESERIALIZE(options.gpcEnabled);
      $(GPC).off('click');

      GPC.onclick = function() {
        this.checked =
            options.gpcEnabled = !DESERIALIZE(options.gpcEnabled);
      };
